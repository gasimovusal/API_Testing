...
public class Post_03 extends AgroMonitoringBaseURL {
...
//        assertEquals(String.valueOf(requestBody.coordinates[0][0][0]), ((Map)((Map)actualDataMap.get("geo_json")).get("geometry")).get("coordinates").toString().substring(3, 12));
        //assertEquals(requestBody.geometrySetUp().get("type"), ((Map)((Map)actualDataMap.get("geo_json")).get("geometry")).get("type").toString().substring(3, 12));
        ...
        assertTrue(json.getString("geo_json.geometry.type").equals(requestBody.geometrySetUp().get("type"))); // for string, we use equals method
        assertTrue(json.getString("geo_json.type").equals(requestBody.geo_json_SetUp().get("type")));
        assertTrue(json.getJsonObject("geo_json.properties").equals(requestBody.geo_json_SetUp().get("properties"))); // properties is empty, thats why you need to use getJsonObject
        assertTrue(json.getString("name").equals(requestBodyMap.get("name")));
        assertTrue(json.getFloat("center[0]")==requestBody.center[0]); // will give you first value in center
        assertTrue(json.getFloat("center[1]")==requestBody.center[1]);

        // To asser "area" value you can use both of them following:
        assertTrue(json.get("area").toString().equals(requestBodyMap.get("area").toString())); // requestBodyMap is an object we cannot use == // I convert
